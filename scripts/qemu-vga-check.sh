...
command=sys.argv[2]
expected='shellrun:' if command in ('shellrun','execpath') else ('init pid/ready' if command=='initinfo' else 'resources as/' if command=='resourceinfo' else ('adoption init/' if command=='adoptioninfo' else ('wait block/' if command=='waitblockinfo' else ('timertest:' if command=='timertest' else ('st: module' if command=='moduletest' else ('mictest:' if command=='lockatomictest' else command+':'))))))
