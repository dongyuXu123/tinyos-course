/* Lesson B09: Mini-GRUB stage2 — MBI 结构构建
 *
...
/* mbi_build: 在 mbi_buf 中构建 MBI（total_size + type-2 tag + end tag），
 * 返回 MBI 物理地址。结构对照 GRUB make_mbi：逐个 append tag、8 对齐、
 * 最后写 total_size。 */
...
void loader_main(void)
{
    ...
    vga_clear();
    vga_puts("B09 Mini-GRUB: load ELF, build MBI, boot Multiboot2 kernel\n");

    if (disk_read_lba(0, KERNEL_LBA, (void *)KERNEL_BUF, KERNEL_SECT) < 0) {
        vga_puts("B09 error: disk read failed\n");
        return;
    }
    if (mb2_header_check((const void *)KERNEL_BUF, KERNEL_SECT * 512u, 0) < 0) {
        vga_puts("B09 error: multiboot2 header rejected\n");
        return;
    }
    if (elf_load((const void *)KERNEL_BUF, KERNEL_SECT * 512u) < 0) {
        vga_puts("B09 error: ELF load failed\n");
        return;
    }
    ...
    vga_hex(mbi_build(), 8);
    vga_puts("\n");
    vga_puts("B09 boot: jumping to kernel...\n");
