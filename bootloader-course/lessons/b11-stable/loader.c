/* Lesson B11: Mini-GRUB stage2 — 装载框架（load/boot 分离）
 *
...
/* vga_goto: 把光标移到指定行（用于把日志放到不会被内核覆盖的区域） */
...
#define KERNEL_LBA   9u            /* 好内核 */
#define BAD_LBA      27u           /* 坏内核（checksum 被破坏） */
#define KERNEL_SECT  18u

#define LOADER_STATE_NONE    0
#define LOADER_STATE_LOADED  1
#define LOADER_STATE_BOOTED   2

static u32 loader_entry;           /* 已装载内核的入口 */
static u32 loader_mbi;             /* 已构建的 MBI 地址 */
static int loader_state = LOADER_STATE_NONE;

/* loader_set: 装载阶段（load）——读盘、校验 header、装载段、构建 MBI，
 * 但不交出控制权。返回 0 成功；负值失败（state 保持 NONE）。
 * 对照 GRUB grub_loader_set：注册装载结果，等待 boot 执行。 */
int loader_set(u32 kernel_lba)
{
    const struct elf32_ehdr *eh;

    mmap_collect();                  /* 装载阶段收集内存图（E820） */
    ...
    loader_entry = eh->e_entry;
    loader_mbi = mbi_build();
    loader_state = LOADER_STATE_LOADED;
    return 0;
}

/* loader_boot: 启动阶段（boot）——校验已有装载结果，执行交接。
 * 对照 GRUB grub_loader_boot：只在装载成功后执行，否则拒绝。 */
int loader_boot(void)
{
    if (loader_state != LOADER_STATE_LOADED)
        return -1;                  /* 没有已装载的内核 */

    loader_state = LOADER_STATE_BOOTED;
    vga_puts("B11 boot: entry=");
    vga_hex(loader_entry, 8);
    vga_puts(" mbi=");
    vga_hex(loader_mbi, 8);
    vga_puts("\n");
    vga_puts("B11 boot: jumping to kernel...\n");

    mb2_boot(loader_entry, loader_mbi);   /* 不返回 */
    return 0;
}
...
void loader_main(void)
{
    ...
    /* 框架演示日志放在屏幕下部（13 行起）：内核启动后从第 0 行输出，
     * 恰好不覆盖这里；上部留给内核与装载日志。 */
    vga_goto(13);
    vga_puts("B11 Mini-GRUB: loader_set / loader_boot framework\n");

    /* 错误路径：坏内核被 load 拒绝，boot 拒绝执行（state 保持 NONE） */
    vga_puts("== try bad kernel ==\n");
    if (loader_set(BAD_LBA) < 0)
        vga_puts("B11 error: bad kernel rejected\n");
    if (loader_boot() < 0)
        vga_puts("B11 error: boot refused (no kernel loaded)\n");

    /* 正确路径：装载好内核并启动 */
    vga_puts("== load good kernel ==\n");
    if (loader_set(KERNEL_LBA) < 0) {
        vga_puts("B11 error: good kernel load failed\n");
        return;
    }
    loader_boot();                     /* 不返回 */
}
