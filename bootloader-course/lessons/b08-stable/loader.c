/* Lesson B08: Mini-GRUB stage2 — PT_LOAD 装载与 Multiboot2 交接
 *
...
/* 最小 MBI：total_size + reserved + end tag（B09 扩展为完整 tag 链） */
struct mbi_min {
    u32 total_size;      /* +0 */
    u32 reserved;        /* +4 */
...
void loader_main(void)
{
    ...
    vga_clear();
    vga_puts("B08 Mini-GRUB: load ELF and boot Multiboot2 kernel\n");

    if (disk_read_lba(0, KERNEL_LBA, (void *)KERNEL_BUF, KERNEL_SECT) < 0) {
        vga_puts("B08 error: disk read failed\n");
        return;
    }
    if (mb2_header_check((const void *)KERNEL_BUF, KERNEL_SECT * 512u, 0) < 0) {
        vga_puts("B08 error: multiboot2 header rejected\n");
        return;
    }
    if (elf_load((const void *)KERNEL_BUF, KERNEL_SECT * 512u) < 0) {
        vga_puts("B08 error: ELF load failed\n");
        return;
    }

    eh = (const struct elf32_ehdr *)KERNEL_BUF;
    vga_puts("B08 boot: entry=");
    vga_hex(eh->e_entry, 8);
    vga_puts(" eax=36d76289 mbi=");
    vga_hex((u32)(u32 *)&mbi, 8);
    vga_puts("\n");
    vga_puts("B08 boot: jumping to kernel...\n");

    mb2_boot(eh->e_entry, (u32)(u32 *)&mbi);   /* 不返回 */
    /* 内核启动后接管屏幕 */
}
