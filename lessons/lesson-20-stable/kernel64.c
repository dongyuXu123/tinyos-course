/* Lesson 20: keyboard wake-one blocking at the IRQ0 return boundary. */
...
/* Thread 0 is the always-runnable shell; workers own one PMM frame each. */
enum thread_state { THREAD_EMPTY, THREAD_RUNNING, THREAD_RUNNABLE, THREAD_SLEEPING, THREAD_BLOCKED_KBD, THREAD_FINISHED };
...
static u64 preempt_switches,quantum_left,sleep_wakeups,idle_worker_ticks,kbd_wait_enqueues,kbd_wake_one,kbd_direct_deliveries;
...
static TEXT64 const char *thread_state_name(u8 state){if(state==THREAD_RUNNING)return "running";if(state==THREAD_RUNNABLE)return "runnable";if(state==THREAD_SLEEPING)return "sleeping";if(state==THREAD_BLOCKED_KBD)return "blocked-kbd";if(state==THREAD_FINISHED)return "finished";return "empty";}
static TEXT64 int tick_due(u64 now,u64 deadline){return (u64)(now-deadline)<(1ULL<<63);}
static TEXT64 void wake_sleepers(void){u32 i;for(i=1;i<THREAD_COUNT;i++)if(threads[i].state==THREAD_SLEEPING&&tick_due(ticks,threads[i].wake_tick)){threads[i].state=THREAD_RUNNABLE;sleep_wakeups++;}}
static TEXT64 int kbd_wait_push(u8 id){if(kbd_wait_count>=THREAD_COUNT-1)return 0;kbd_waiters[kbd_wait_head]=id;kbd_wait_head=(u8)((kbd_wait_head+1)%(THREAD_COUNT-1));kbd_wait_count++;kbd_wait_enqueues++;return 1;}
static TEXT64 int kbd_wait_pop(u8 *id){if(!kbd_wait_count)return 0;*id=kbd_waiters[kbd_wait_tail];kbd_wait_tail=(u8)((kbd_wait_tail+1)%(THREAD_COUNT-1));kbd_wait_count--;return 1;}
...
static TEXT64 void reap_finished_threads(void){u32 i;for(i=1;i<THREAD_COUNT;i++)if(i!=current_thread&&threads[i].state==THREAD_FINISHED&&threads[i].stack_phys){u64 p=threads[i].stack_phys;threads[i].stack_phys=0;(void)pmm_free_page(p);}}
/* Called from IRQ0 only. It returns the exact frame restored by IRQ0's one iretq path. */
TEXT64 struct irq0_frame *irq0_schedule(struct irq0_frame *f){u8 old,next;ticks++;outb64(PIC1_COMMAND,PIC_EOI);threads[current_thread].frame=(u64)(unsigned long)f;wake_sleepers();reap_finished_threads();if(!threads_started)return f;if(quantum_left)quantum_left--;if(quantum_left)return f;old=current_thread;next=next_runnable();quantum_left=TIME_SLICE_TICKS;if(next==old){if(old==0)idle_worker_ticks++;return f;}if(threads[old].state==THREAD_RUNNING)threads[old].state=THREAD_RUNNABLE;threads[next].state=THREAD_RUNNING;current_thread=next;threads[next].switches++;preempt_switches++;return (struct irq0_frame *)(unsigned long)threads[next].frame;}
static TEXT64 void busy_delay(void){volatile u64 n;for(n=0;n<BUSY_SPINS;n++)__asm__ volatile("":::"memory");}
static TEXT64 void thread_sleep_ticks(u64 delta){u64 flags;u8 id=current_thread;if(!delta)delta=1;flags=irq_save64();if(id&&threads[id].state==THREAD_RUNNING){threads[id].wake_tick=ticks+delta;threads[id].state=THREAD_SLEEPING;}irq_restore64(flags);while(threads[id].state==THREAD_SLEEPING)__asm__ volatile("sti; hlt");}
static TEXT64 void kbd_wait_char(u8 *out){u8 id=current_thread;for(;;){u64 flags=irq_save64();if(id&&threads[id].mailbox_ready){*out=threads[id].mailbox;threads[id].mailbox_ready=0;irq_restore64(flags);return;}if(id&&threads[id].state==THREAD_RUNNING&&kbd_wait_push(id))threads[id].state=THREAD_BLOCKED_KBD;irq_restore64(flags);while(threads[id].state==THREAD_BLOCKED_KBD)__asm__ volatile("sti; hlt");}}
static TEXT64 void worker_run(u8 id){while(threads[id].progress<THREAD_STEPS){if(kbd_wait_test){u8 ch;kbd_wait_char(&ch);threads[id].mailbox=ch;threads[id].received++;}threads[id].progress++;if(sleep_test)thread_sleep_ticks(id==1?SLEEP_A_TICKS:SLEEP_B_TICKS);else if(!kbd_wait_test)busy_delay();}thread_exit();}
...
TEXT64 void irq1_record(void){u8 raw=inb64(0x60),ch,next,id;irq1_last_scancode=raw;irq1_raw_count++;if(!(raw&0x80)){irq1_count++;ch=(u8)scan64(raw);if(ch){if(kbd_wait_pop(&id)&&id&&id<THREAD_COUNT&&threads[id].state==THREAD_BLOCKED_KBD){threads[id].mailbox=ch;threads[id].mailbox_ready=1;threads[id].state=THREAD_RUNNABLE;kbd_wake_one++;kbd_direct_deliveries++;}else{next=(u8)((kbd_head+1)&(KBD_QUEUE_SIZE-1));if(next==kbd_tail)kbd_overflow_count++;else{kbd_queue[kbd_head]=ch;kbd_head=next;}}}}outb64(PIC1_COMMAND,PIC_EOI);}
