/* Lesson 17: cooperative threads atop the Lesson 16 high runtime alias. */
...
static TEXT64 void yield_thread(void);
...
static TEXT64 void worker_run(u8 id){while(threads[id].progress<THREAD_STEPS){threads[id].progress++;yield_thread();}}
TEXT64 void thread_trampoline_c(void){u8 id=current_thread;__asm__ volatile("sti":::"memory");worker_run(id);thread_exit();}
static TEXT64 void thread_exit(void){threads[current_thread].state=THREAD_FINISHED;yield_thread();for(;;)__asm__ volatile("cli; hlt");}
