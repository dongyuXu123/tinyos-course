...
#define BOCHS_WIDTH 800U
#define BOCHS_HEIGHT 600U
#define BOCHS_PITCH (BOCHS_WIDTH*4U)
...
static u64 bochs_lfb;
static u64 bootstrap_cursor, bootstrap_end;

static int ranges_overlap(u64 a,u64 b,u64 c,u64 d) { return a < d && c < b; }
static void pci_cfg_write(u32 address,u32 value){volatile u32*p=(volatile u32 *)(unsigned long)0xcfc;__asm__ volatile("outl %0,%1"::"a"(address),"Nd"((u16)0xcf8));__asm__ volatile("outl %0,%1"::"a"(value),"Nd"((u16)0xcfc));(void)p;}
static u32 pci_cfg_read(u32 address){u32 value;__asm__ volatile("outl %0,%1"::"a"(address),"Nd"((u16)0xcf8));__asm__ volatile("inl %1,%0":"=a"(value):"Nd"((u16)0xcfc));return value;}
static int bochs_pci_init(void){u32 dev,addr,id,bar,cmd;for(dev=0;dev<32;dev++){addr=0x80000000U|(dev<<11);id=pci_cfg_read(addr);if((id&0xffffU)!=0x1234U||((id>>16)&0xffffU)!=0x1111U)continue;pci_cfg_write(addr|0x10,0xfc000000U);pci_cfg_write(addr|0x18,0xfebdc000U);cmd=pci_cfg_read(addr|4);pci_cfg_write(addr|4,cmd|6U);bar=pci_cfg_read(addr|0x10)&0xfffffff0U;if(!bar)continue;bochs_lfb=bar;return 1;}return 0;}
...
static int prepare_memory_map(void)
{
    ...
    while(pos<end) { const struct mb2_tag *tag; u32 rounded;
        ...
        if(tag->type==MB2_TAG_END) {
            u64 bytes;
            if(tag->size!=8 || !memory_map) return 0;
            if(framebuffer_tag && framebuffer_tag->size>=32 &&
               ...
               framebuffer_tag->address+bytes>=framebuffer_tag->address) {
                ...
                long_mode_handoff.framebuffer_green_pos=framebuffer_tag->size>=38?framebuffer_tag->green_pos:8;
                long_mode_handoff.framebuffer_green_mask=framebuffer_tag->size>=38?framebuffer_tag->green_mask:8;
                long_mode_handoff.framebuffer_blue_pos=framebuffer_tag->size>=38?framebuffer_tag->blue_pos:0;
                long_mode_handoff.framebuffer_blue_mask=framebuffer_tag->size>=38?framebuffer_tag->blue_mask:8;
                long_mode_handoff.framebuffer_map=FRAMEBUFFER_VA;
                framebuffer_ready=1;
            } else {
                /* Use the PCI BAR assigned by the kernel, never a guessed address. */
                if(!bochs_pci_init()){framebuffer_ready=0;memory_map_ready=1;return 1;}
                long_mode_handoff.framebuffer_address=bochs_lfb;
                long_mode_handoff.framebuffer_pitch=BOCHS_PITCH;
                long_mode_handoff.framebuffer_width=BOCHS_WIDTH;
                long_mode_handoff.framebuffer_height=BOCHS_HEIGHT;
                long_mode_handoff.framebuffer_bpp=32;
                long_mode_handoff.framebuffer_type=1;
                long_mode_handoff.framebuffer_bytes=BOCHS_PITCH*BOCHS_HEIGHT;
                long_mode_handoff.framebuffer_red_pos=16;
                long_mode_handoff.framebuffer_red_mask=8;
                long_mode_handoff.framebuffer_green_pos=8;
                long_mode_handoff.framebuffer_green_mask=8;
                long_mode_handoff.framebuffer_blue_pos=0;
                long_mode_handoff.framebuffer_blue_mask=8;
                long_mode_handoff.framebuffer_phys_base=bochs_lfb;
                long_mode_handoff.framebuffer_map_offset=0;
                long_mode_handoff.framebuffer_map=FRAMEBUFFER_VA;
                framebuffer_ready=1;
            }
            memory_map_ready=1;
...
static u32 setup_long_mode_tables(void)
{
    ...
    if(framebuffer_ready){u32 first=(u32)(FRAMEBUFFER_VA/(PAGE_ENTRIES*PAGE_SIZE)),entry=(u32)((FRAMEBUFFER_VA/PAGE_SIZE)%PAGE_ENTRIES),pages=(long_mode_handoff.framebuffer_map_offset+long_mode_handoff.framebuffer_bytes+PAGE_SIZE-1)/PAGE_SIZE,k;for(k=0;k<pages;k++){u32 slot=first+(entry+k)/PAGE_ENTRIES,off=(entry+k)%PAGE_ENTRIES;if(slot>=PAGE_TABLES_PER_ALIAS){framebuffer_ready=0;break;}((volatile u64*)(unsigned long)(u32)long_mode_handoff.pt[slot])[off]=(long_mode_handoff.framebuffer_phys_base+(u64)k*PAGE_SIZE)|PTE_PRESENT_WRITABLE;((volatile u64*)(unsigned long)(u32)long_mode_handoff.high_pt[slot])[off]=(long_mode_handoff.framebuffer_phys_base+(u64)k*PAGE_SIZE)|PTE_PRESENT_WRITABLE;}}
...
u32 kernel_main32(u32 magic,u32 mbi_address)
{
    ...
    if(!prepare_memory_map()){ image_failure_report(8); return 0; }
    if(framebuffer_ready){
        long_mode_handoff.framebuffer_phys_base=
            align_down_page(long_mode_handoff.framebuffer_address);
        long_mode_handoff.framebuffer_map_offset=
            (u32)(long_mode_handoff.framebuffer_address-
                  long_mode_handoff.framebuffer_phys_base);
        long_mode_handoff.framebuffer_map=
            FRAMEBUFFER_VA+long_mode_handoff.framebuffer_map_offset;
    }
    long_mode_handoff.mbi_address=multiboot_address; long_mode_handoff.mbi_size=multiboot_total_size;
    { u32 result=setup_long_mode_tables(); if(!result) image_failure_report(9); return result; }
}
