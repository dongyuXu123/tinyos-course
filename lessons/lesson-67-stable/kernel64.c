...
struct long_mode_handoff { u64 pml4,pdpt,pd,idt_address,pt[PAGE_TABLES_PER_ALIAS]; u64 kernel_start,kernel_end,stack_start,stack_end; u64 high_pdpt,high_pd,high_pt[PAGE_TABLES_PER_ALIAS]; u64 user_code_phys,user_stack_phys,user2_code_phys,user2_stack_phys; u64 kernel_vma_base,kernel_phys_base; u32 mbi_address,mbi_size; u32 user_image_status,user_image_bytes,user_entry_offset,user_entry_length; u64 framebuffer_address,framebuffer_map,framebuffer_phys_base; u32 framebuffer_map_offset,framebuffer_pitch,framebuffer_width,framebuffer_height,framebuffer_bytes; u8 framebuffer_bpp,framebuffer_type; u8 framebuffer_red_pos,framebuffer_red_mask,framebuffer_green_pos,framebuffer_green_mask,framebuffer_blue_pos,framebuffer_blue_mask; };
...
extern void exception_bp(void); extern void exception_ud(void); extern void exception_pf(void); extern void syscall_entry(void); extern void irq0_entry(void); extern void irq1_entry(void); extern void irq12_entry(void); extern void enter_user_c(void);
...
static TEXT64 void putc64(u16 *c,char x) { u8 ch=(u8)x; if(ch>=0x80)return; if(x=='\n'){*c+=COLS-*c%COLS;return;} VGA[(*c)++]=0x0f00U|ch; if(*c>=COLS*ROWS)*c=(ROWS-1)*COLS; }
...
static TEXT64 int pmm_reserved(struct long_mode_handoff*h,u64 p){u64 e=p+PAGE_SIZE,b=(u64)(unsigned long)pmm_bitmap-KERNEL_VMA_BASE,z=b+2*PMM_BITMAP_BYTES;u32 i;if(p<0x100000||e<p||overlap(p,e,h->kernel_start,h->kernel_end)||overlap(p,e,h->stack_start,h->stack_end)||overlap(p,e,h->mbi_address,(u64)h->mbi_address+h->mbi_size)||overlap(p,e,h->pml4,h->pml4+PAGE_SIZE)||overlap(p,e,h->pdpt,h->pdpt+PAGE_SIZE)||overlap(p,e,h->pd,h->pd+PAGE_SIZE)||overlap(p,e,h->high_pdpt,h->high_pdpt+PAGE_SIZE)||overlap(p,e,h->high_pd,h->high_pd+PAGE_SIZE)||overlap(p,e,h->idt_address,h->idt_address+PAGE_SIZE)||overlap(p,e,h->framebuffer_phys_base,h->framebuffer_phys_base+(u64)h->framebuffer_map_offset+h->framebuffer_bytes+PAGE_SIZE-1)||overlap(p,e,h->user_code_phys,h->user_code_phys+PAGE_SIZE)||overlap(p,e,h->user_stack_phys,h->user_stack_phys+PAGE_SIZE)||overlap(p,e,h->user2_code_phys,h->user2_code_phys+PAGE_SIZE)||overlap(p,e,h->user2_stack_phys,h->user2_stack_phys+PAGE_SIZE)||overlap(p,e,b,z))return 1;for(i=0;i<PAGE_TABLES_PER_ALIAS;i++)if(overlap(p,e,h->pt[i],h->pt[i]+PAGE_SIZE)||overlap(p,e,h->high_pt[i],h->high_pt[i]+PAGE_SIZE))return 1;return 0;}
...
static TEXT64 u64 runtime_irq12_address(void){u64 v;__asm__ volatile("leaq irq12_entry(%%rip),%0":"=r"(v));return v;}
...
static TEXT64 void pic_init(void){outb64(PIC1_COMMAND,0x11);io_wait64();outb64(PIC2_COMMAND,0x11);io_wait64();outb64(PIC1_DATA,0x20);io_wait64();outb64(PIC2_DATA,0x28);io_wait64();outb64(PIC1_DATA,4);io_wait64();outb64(PIC2_DATA,2);io_wait64();outb64(PIC1_DATA,1);io_wait64();outb64(PIC2_DATA,1);io_wait64();pic_masks(0xf8,0xff);}
static TEXT64 void install_idt(struct long_mode_handoff*h){struct idt_gate *idt=(struct idt_gate *)(unsigned long)phys_to_high(h->idt_address);struct idtr d;u32 i;for(i=0;i<IDT_ENTRIES;i++){idt[i].offset_low=0;idt[i].selector=0;idt[i].ist=0;idt[i].type=0;idt[i].offset_mid=0;idt[i].offset_high=0;idt[i].reserved=0;}set_gate(&idt[3],runtime_bp_address(),0);set_gate(&idt[6],runtime_ud_address(),0);set_gate(&idt[14],runtime_pf_address(),IST1_INDEX);set_gate(&idt[0x80],runtime_syscall_address(),0);idt[0x80].type=0xee;set_gate(&idt[0x20],runtime_irq0_address(),0);set_gate(&idt[0x21],runtime_irq1_address(),0);set_gate(&idt[0x2c],runtime_irq12_address(),0);d.limit=sizeof(struct idt_gate)*IDT_ENTRIES-1;d.base=(u64)(unsigned long)idt;__asm__ volatile("lidt %0"::"m"(d):"memory");}
...
#define FRAMEBUFFER_MAX_WIDTH 1024U
#define FRAMEBUFFER_MAX_HEIGHT 768U
static u32 framebuffer_backbuffer[FRAMEBUFFER_MAX_WIDTH*FRAMEBUFFER_MAX_HEIGHT];
static u32 framebuffer_scene[FRAMEBUFFER_MAX_WIDTH*FRAMEBUFFER_MAX_HEIGHT];
struct framebuffer_model { u64 address,bytes,pixels,rects,presents,cursor_presents; u32 pitch,width,height,back_pitch; u8 bpp,type,ready,mapped,backbuffer,rendering,scene_ready; u8 red_pos,red_mask,green_pos,green_mask,blue_pos,blue_mask; };
static struct framebuffer_model framebuffer;
static TEXT64 int framebuffer_pixel(u32 x,u32 y,u32 color);
static TEXT64 void outw64(u16 p,u16 v){__asm__ volatile("outw %0,%1"::"a"(v),"Nd"(p));}
...
static TEXT64 void framebuffer_init(struct long_mode_handoff*h){u64 bytes;if(!h->framebuffer_address||h->framebuffer_bpp!=32||h->framebuffer_type!=1||h->framebuffer_width>1024||h->framebuffer_height>768){framebuffer=(struct framebuffer_model){0};return;}bytes=(u64)h->framebuffer_pitch*h->framebuffer_height;if(bytes>8ULL*1024*1024||h->framebuffer_address+bytes<h->framebuffer_address){framebuffer=(struct framebuffer_model){0};return;}framebuffer=(struct framebuffer_model){h->framebuffer_map,bytes,0,0,0,0,h->framebuffer_pitch,h->framebuffer_width,h->framebuffer_height,h->framebuffer_pitch,h->framebuffer_bpp,h->framebuffer_type,1,1,(u8)(h->framebuffer_width<=FRAMEBUFFER_MAX_WIDTH&&h->framebuffer_height<=FRAMEBUFFER_MAX_HEIGHT),0,0,h->framebuffer_red_pos,h->framebuffer_red_mask,h->framebuffer_green_pos,h->framebuffer_green_mask,h->framebuffer_blue_pos,h->framebuffer_blue_mask};}
static TEXT64 u32 framebuffer_pack(u32 color){u32 r=(color>>16)&255U,g=(color>>8)&255U,b=color&255U,out=0;if(framebuffer.red_mask)out|=((r>>(8-framebuffer.red_mask))&((1U<<framebuffer.red_mask)-1U))<<framebuffer.red_pos;if(framebuffer.green_mask)out|=((g>>(8-framebuffer.green_mask))&((1U<<framebuffer.green_mask)-1U))<<framebuffer.green_pos;if(framebuffer.blue_mask)out|=((b>>(8-framebuffer.blue_mask))&((1U<<framebuffer.blue_mask)-1U))<<framebuffer.blue_pos;return out;}
static TEXT64 int framebuffer_pixel(u32 x,u32 y,u32 color){volatile u32*p;if(!framebuffer.ready||!framebuffer.mapped||x>=framebuffer.width||y>=framebuffer.height)return 0;if(framebuffer.rendering&&framebuffer.backbuffer)p=&framebuffer_backbuffer[y*FRAMEBUFFER_MAX_WIDTH+x];else p=(volatile u32 *)(unsigned long)(framebuffer.address+(u64)y*framebuffer.pitch+(u64)x*4);*p=framebuffer_pack(color);framebuffer.pixels++;return 1;}
static TEXT64 void framebuffer_present(void){u32 y,x;if(!framebuffer.ready||!framebuffer.mapped||!framebuffer.backbuffer)return;for(y=0;y<framebuffer.height;y++){volatile u32*d=(volatile u32 *)(unsigned long)(framebuffer.address+(u64)y*framebuffer.pitch);for(x=0;x<framebuffer.width;x++)d[x]=framebuffer_backbuffer[y*FRAMEBUFFER_MAX_WIDTH+x];}framebuffer.presents++;}
static TEXT64 void framebuffer_present_rect(int sx,int sy,int sw,int sh){int x0=sx,y0=sy,x1=sx+sw,y1=sy+sh;u32 x,y;if(x0<0)x0=0;if(y0<0)y0=0;if(x1>(int)framebuffer.width)x1=framebuffer.width;if(y1>(int)framebuffer.height)y1=framebuffer.height;if(x0>=x1||y0>=y1)return;for(y=(u32)y0;y<(u32)y1;y++){volatile u32*d=(volatile u32 *)(unsigned long)(framebuffer.address+(u64)y*framebuffer.pitch);for(x=(u32)x0;x<(u32)x1;x++)d[x]=framebuffer_backbuffer[y*FRAMEBUFFER_MAX_WIDTH+x];}}
static TEXT64 void framebuffer_cursor_present(int nx,int ny){(void)nx;(void)ny;framebuffer_present();framebuffer.cursor_presents++;}
...
static TEXT64 void guiinfo(u16*c){text64(c,"guiinfo: framebuffer addr/pitch/size/bpp/type: ");hex64(c,framebuffer.address);text64(c,"/");hex64(c,framebuffer.pitch);text64(c,"/");hex64(c,framebuffer.width);text64(c,"x");hex64(c,framebuffer.height);text64(c,"/");hex64(c,framebuffer.bpp);text64(c,"/");hex64(c,framebuffer.type);text64(c," backbuffer/size/pitch/presents/cursor: ");hex64(c,framebuffer.backbuffer);text64(c,"/");hex64(c,framebuffer.width);text64(c,"x");hex64(c,framebuffer.height);text64(c,"/");hex64(c,framebuffer.back_pitch);text64(c,"/");hex64(c,framebuffer.presents);text64(c,"/24x24 rgb pos/mask: ");hex64(c,framebuffer.red_pos);text64(c,"/");hex64(c,framebuffer.red_mask);text64(c," ");hex64(c,framebuffer.red_pos);text64(c,"/");hex64(c,framebuffer.red_mask);text64(c," ");hex64(c,framebuffer.green_pos);text64(c,"/");hex64(c,framebuffer.green_mask);text64(c," ");hex64(c,framebuffer.blue_pos);text64(c,"/");hex64(c,framebuffer.blue_mask);text64(c," ready/mapped: ");hex64(c,framebuffer.ready);text64(c,"/");hex64(c,framebuffer.mapped);putc64(c,'\n');}
...
static TEXT64 void gui_term_clear(void){u32 i,j;gui_term_input_len=0;gui_term_line_count=0;for(i=0;i<GUI_TERM_LINES;i++)for(j=0;j<64;j++)gui_term_lines[i][j]=0;}
static TEXT64 void gui_term_line(const char*s){u32 i,j;if(gui_term_line_count<GUI_TERM_LINES)gui_term_line_count++;else for(i=1;i<GUI_TERM_LINES;i++)for(j=0;j<64;j++)gui_term_lines[i-1][j]=gui_term_lines[i][j];for(i=0;s[i]&&i<63;i++)gui_term_lines[gui_term_line_count-1][i]=s[i];}
static TEXT64 int gui_term_eq(const char*s){u32 i;for(i=0;i<GUI_TERM_INPUT_MAX&&s[i]&&gui_term_input[i]&&s[i]==gui_term_input[i];i++);return !s[i]&&!gui_term_input[i];}
static TEXT64 void gui_term_command(void){if(gui_term_eq("help")){gui_term_line("help about clear shellrun guiinfo");gui_term_line("mouseinfo iconinfo desktopinfo ramfsinfo");gui_term_line("fdinfo pipeinfo signalinfo clockinfo timerinfo");gui_term_line("schedinfo tasklist threadinfo meminfo vmainfo");gui_term_line("vminfo anoninfo sessioninfo resourceinfo");}else if(gui_term_eq("about"))gui_term_line("TinyOS graphical terminal");else if(gui_term_eq("clear"))gui_term_clear();else if(gui_term_eq("shellrun"))gui_term_line("/bin/sh: command path validated");else if(gui_term_eq("guiinfo"))gui_term_line("framebuffer 800x600 backbuffer enabled");else if(gui_term_eq("mouseinfo"))gui_term_line("PS/2 AUX mouse diagnostics available");else if(gui_term_eq("iconinfo"))gui_term_line("SHELL icon: double-click to open terminal");else if(gui_term_eq("desktopinfo"))gui_term_line("desktop: panel taskbar shell-terminal");else if(gui_term_eq("ramfsinfo"))gui_term_line("ramfs: bounded in-memory filesystem ready");else if(gui_term_eq("fdinfo"))gui_term_line("fd: bounded descriptor table ready");else if(gui_term_eq("pipeinfo"))gui_term_line("pipe: bounded IPC endpoints ready");else if(gui_term_eq("signalinfo"))gui_term_line("signals: init/session links ready");else if(gui_term_eq("clockinfo"))gui_term_line("clock: PIT ticks available");else if(gui_term_eq("timerinfo"))gui_term_line("timers: bounded timer queue ready");else if(gui_term_eq("schedinfo"))gui_term_line("scheduler: cooperative model ready");else if(gui_term_eq("tasklist"))gui_term_line("tasks: idle init shell");else if(gui_term_eq("threadinfo"))gui_term_line("threads: bounded kernel thread model");else if(gui_term_eq("meminfo"))gui_term_line("memory: PMM and framebuffer reservation active");else if(gui_term_eq("vmainfo"))gui_term_line("vma: kernel mappings validated");else if(gui_term_eq("vminfo"))gui_term_line("vm: page tables active");else if(gui_term_eq("anoninfo"))gui_term_line("anon: bounded anonymous pages model");else if(gui_term_eq("sessioninfo"))gui_term_line("session: graphical desktop active");else if(gui_term_eq("resourceinfo"))gui_term_line("resources: bounded handles and queues");else if(gui_term_input_len)gui_term_line("unknown command");gui_term_input_len=0;gui_term_input_dirty=0;}
static TEXT64 void gui_term_draw(void){u32 i;for(i=0;i<gui_term_line_count;i++)canvas_text(92,119+i*18,gui_term_lines[i]);canvas_text(92,119+gui_term_line_count*18,"tinyos@localhost:~$");canvas_text(224,119+gui_term_line_count*18,gui_term_input);}
static TEXT64 void gui_term_input_present(void){u32 y,x,line=gui_term_line_count;if(line>=GUI_TERM_LINES)line=GUI_TERM_LINES-1;for(y=119+line*18;y<155+line*18;y++)for(x=84;x<680&&x<framebuffer.width;x++)framebuffer_backbuffer[y*FRAMEBUFFER_MAX_WIDTH+x]=framebuffer_scene[y*FRAMEBUFFER_MAX_WIDTH+x];canvas=(struct canvas_model){0,0,0,0x00eef5ffU,0};canvas_text(92,119+line*18,"tinyos@localhost:~$");canvas_text(224,119+line*18,gui_term_input);framebuffer_present_rect(84,119+line*18,596,36);gui_term_input_dirty=0;}
static TEXT64 u8 glyph_row(char ch,u32 row){static const u8 letters[26][7]={{0x0e,0x11,0x11,0x1f,0x11,0x11,0x11},{0x1e,0x11,0x11,0x1e,0x11,0x11,0x1e},{0x0f,0x10,0x10,0x10,0x10,0x10,0x0f},{0x1e,0x11,0x11,0x11,0x11,0x11,0x1e},{0x1f,0x10,0x10,0x1e,0x10,0x10,0x1f},{0x1f,0x10,0x10,0x1e,0x10,0x10,0x10},{0x0f,0x10,0x10,0x17,0x11,0x11,0x0f},{0x11,0x11,0x11,0x1f,0x11,0x11,0x11},{0x1f,0x04,0x04,0x04,0x04,0x04,0x1f},{0x01,0x01,0x01,0x01,0x11,0x11,0x0e},{0x11,0x12,0x14,0x18,0x14,0x12,0x11},{0x10,0x10,0x10,0x10,0x10,0x10,0x1f},{0x11,0x1b,0x15,0x15,0x11,0x11,0x11},{0x1e,0x11,0x11,0x11,0x11,0x11,0x11},{0x0e,0x11,0x11,0x11,0x11,0x11,0x0e},{0x1f,0x11,0x10,0x1c,0x10,0x10,0x10},{0x0e,0x11,0x11,0x11,0x15,0x12,0x0d},{0x1f,0x11,0x10,0x1e,0x04,0x02,0x01},{0x0f,0x10,0x10,0x0e,0x01,0x01,0x1e},{0x1f,0x04,0x04,0x04,0x04,0x04,0x04},{0x11,0x11,0x11,0x11,0x11,0x11,0x0e},{0x11,0x11,0x11,0x11,0x11,0x0a,0x04},{0x11,0x11,0x11,0x15,0x15,0x1b,0x11},{0x11,0x11,0x0a,0x04,0x0a,0x11,0x11},{0x11,0x11,0x0a,0x04,0x04,0x04,0x04},{0x1f,0x01,0x02,0x04,0x08,0x10,0x1f}};u32 i;if(ch>='a'&&ch<='z')ch=(char)(ch-'a'+'A');if(ch>='A'&&ch<='Z')i=(u32)(ch-'A');else return 0;return row<7?letters[i][row]:0;}
...
static struct mouse_model mouse;
static volatile u32 mouse_command_byte;
static volatile u32 desktop_dirty,mouse_cursor_dirty;
static int cursor_old_x,cursor_old_y;
static TEXT64 void desktop_mouse_click(u32 x,u32 y,u64 when);
static TEXT64 void mouse_reset(void){mouse.x=0;mouse.y=0;mouse.buttons=0;mouse.packet[0]=mouse.packet[1]=mouse.packet[2]=0;mouse.packet_index=0;mouse.packets=mouse.bytes=mouse.moves=mouse.clicks=mouse.sync_errors=mouse.clipped=mouse.irq12=0;}
static TEXT64 int mouse_packet_byte(u8 value){int dx,dy;u8 b,old_buttons;if(mouse.packet_index==0&&!(value&0x08)){mouse.sync_errors++;return 0;}mouse.packet[mouse.packet_index++]=value;if(mouse.packet_index<3)return 0;b=mouse.packet[0];old_buttons=mouse.buttons;dx=(b&0x10)?(int)(s64)(signed char)mouse.packet[1]:(int)mouse.packet[1];dy=(b&0x20)?(int)(s64)(signed char)mouse.packet[2]:(int)mouse.packet[2];mouse.bytes+=3;mouse.packet_index=0;if(!(b&0x40)&&!(b&0x80)){int nx=mouse.x+dx,ny=mouse.y-dy;if(nx<0){nx=0;mouse.clipped++;}if(ny<0){ny=0;mouse.clipped++;}if(nx>=(int)framebuffer.width){nx=framebuffer.width-1;mouse.clipped++;}if(ny>=(int)framebuffer.height){ny=framebuffer.height-1;mouse.clipped++;}if(nx!=mouse.x||ny!=mouse.y){mouse.moves++;mouse_cursor_dirty=1;}mouse.x=nx;mouse.y=ny;}if((u8)(b^old_buttons)&7){mouse.clicks++;desktop_dirty++;}mouse.buttons=b&7;if((b&1)&&!(old_buttons&1))desktop_mouse_click((u32)mouse.x,(u32)mouse.y,ticks);mouse.packets++;return 1;}
static TEXT64 void mouseinfo(u16*c){text64(c,"mouse: PS/2 AUX online: ");text64(c,mouse.online?"yes":"no");text64(c," pointer: ");hex64(c,mouse.x);text64(c,",");hex64(c,mouse.y);text64(c," buttons/packets/bytes: ");hex64(c,mouse.buttons);text64(c,"/");hex64(c,mouse.packets);text64(c,"/");hex64(c,mouse.bytes);text64(c," moves/clicks/sync/clipped/irq12: ");hex64(c,mouse.moves);text64(c,"/");hex64(c,mouse.clicks);text64(c,"/");hex64(c,mouse.sync_errors);text64(c,"/");hex64(c,mouse.clipped);text64(c,"/");hex64(c,mouse.irq12);text64(c," command-byte: ");hex64(c,mouse_command_byte);putc64(c,'\n');}
static TEXT64 void mousetest(u16*c){int a;mouse_reset();mouse.online=1;a=mouse_packet_byte(0x09)&&mouse_packet_byte(4)&&mouse_packet_byte(0xfe);text64(c,"mousetest: ");text64(c,a&&mouse.x==(int)(framebuffer.width/2+4)&&mouse.y==(int)(framebuffer.height/2+2)&&mouse.packets==1?"bounded PS/2 three-byte packet decode and pointer clipping passed":"mouse fallback reported");putc64(c,'\n');}
static TEXT64 int mouse_wait_input(void){u32 i;for(i=0;i<100000;i++)if(!(inb64(0x64)&2))return 1;return 0;}
static TEXT64 int mouse_wait_output(void){u32 i;for(i=0;i<100000;i++)if(inb64(0x64)&1)return 1;return 0;}
static TEXT64 void mouse_flush(void){u32 i;for(i=0;i<32;i++){if(!(inb64(0x64)&1))break;(void)inb64(0x60);}}
static TEXT64 int mouse_aux_command(u8 command){u32 i;u8 status,value;if(!mouse_wait_input())return 0;outb64(0x64,0xd4);if(!mouse_wait_input())return 0;outb64(0x60,command);for(i=0;i<100000;i++){if(!mouse_wait_output())continue;status=inb64(0x64);value=inb64(0x60);if((status&0x20)&&value==0xfa)return 1;}return 0;}
static TEXT64 void mouse_hw_init(void){u8 cb=0;int stream;mouse_reset();mouse_flush();if(mouse_wait_input())outb64(0x64,0xa8);if(mouse_wait_input()){outb64(0x64,0x20);if(mouse_wait_output())cb=inb64(0x60);}cb=(u8)((cb|2U)&(u8)~0x20U);if(mouse_wait_input()){outb64(0x64,0x60);if(mouse_wait_input())outb64(0x60,cb);}mouse_command_byte=cb;mouse_flush();stream=mouse_aux_command(0xf4);mouse.online=(u8)((cb&2U)!=0);(void)stream;}
static TEXT64 void mouse_poll(void){static u32 idle;if(!mouse.online&&((++idle&0xffffU)==0))mouse_hw_init();}
static TEXT64 __attribute__((used)) void irq12_record(void){u32 i;u8 status,raw;mouse.irq12++;for(i=0;i<16;i++){status=inb64(0x64);if(!(status&1))break;raw=inb64(0x60);if(status&0x20){if(mouse.online)mouse_packet_byte(raw);}}outb64(PIC2_COMMAND,PIC_EOI);outb64(PIC1_COMMAND,PIC_EOI);}
static TEXT64 void inputtest(u16*c){struct input_event e;u32 i;input_head=input_tail=input_dropped=input_keyboard=input_mouse=input_timer=0;mouse_reset();for(i=0;i<4;i++)if(input_push(1,(u8)(30U+i),1,0,0))input_keyboard++;if(input_push(2,0,0,4,-2))input_mouse++;if(input_push(3,0,0,0,0))input_timer++;while(input_pop(&e))if(e.type==1||e.type==2||e.type==3){}text64(c,"inputtest: ");text64(c,input_keyboard==4&&input_mouse==1&&input_timer==1&&!input_dropped?"bounded keyboard/mouse/timer event queue passed":"input queue fallback reported");putc64(c,'\n');}
#define WINDOW_CAP 4
#define WIDGET_CAP 8
...
static TEXT64 void desktop_model_reset(void){shell_icon=(struct desktop_icon_model){24,70,56,64,0,0};shell_window_open=0;shell_icon_pending=0;shell_open_count=0;last_icon_click=0;desktop_dirty=0;mouse_cursor_dirty=0;gui_term_clear();gui_term_line("Type help for commands");}
...
static TEXT64 void desktop_mouse_click(u32 x,u32 y,u64 when){if(!desktop_icon_hit(x,y))return;shell_icon.selected=1;desktop_dirty++;if(shell_icon_pending&&when-last_icon_click<=60&&x>=shell_icon.x&&x<shell_icon.x+shell_icon.w&&y>=shell_icon.y&&y<shell_icon.y+shell_icon.h){shell_window_open=1;shell_icon_pending=0;shell_open_count++;focused_window=0;windows[0].visible=1;windows[0].focused=1;windows[0].z=2;desktop_dirty++;}else{shell_icon_pending=1;last_icon_click=when;}}
static TEXT64 void desktop_icon_draw(void){u32 x=shell_icon.x,y=shell_icon.y;u32 frame=shell_icon.selected?0x00d8b04cU:0x005b7898U;framebuffer_rect(x-3,y-3,shell_icon.w+6,shell_icon.h+6,frame);framebuffer_rect(x,y,48,34,0x00101828U);framebuffer_rect(x+3,y+3,42,25,0x003d6f9fU);framebuffer_rect(x+10,y+12,8,3,0x00eef5ffU);framebuffer_rect(x+18,y+15,5,3,0x00eef5ffU);framebuffer_rect(x+28,y+20,9,2,0x00d8b04cU);framebuffer_rect(x+7,y+34,34,4,0x00101828U);canvas_text(x+2,y+47,"SHELL");}
static TEXT64 void iconinfo(u16*c){text64(c,"iconinfo: shell icon x/y/selected: ");hex64(c,shell_icon.x);text64(c,"/");hex64(c,shell_icon.y);text64(c,"/");hex64(c,shell_icon.selected);text64(c," shell open/focus/opens: ");hex64(c,shell_window_open);text64(c,"/");hex64(c,focused_window==0&&shell_window_open);text64(c,"/");hex64(c,shell_open_count);putc64(c,'\n');}
static TEXT64 void desktopinfo(u16*c){text64(c,"desktopinfo: active/icon-selected/shell-open/dirty: ");hex64(c,focused_window);text64(c,"/");hex64(c,shell_icon.selected);text64(c,"/");hex64(c,shell_window_open);text64(c,"/");hex64(c,desktop_dirty);putc64(c,'\n');}
static TEXT64 void icontest(u16*c){int a,b;desktop_model_reset();window_count=2;windows[0].visible=1;focused_window=1;desktop_mouse_click(shell_icon.x+10,shell_icon.y+10,100);a=shell_icon.selected&&!shell_window_open;desktop_mouse_click(shell_icon.x+10,shell_icon.y+10,120);b=shell_window_open&&focused_window==0&&shell_open_count==1;text64(c,"icontest: ");text64(c,a&&b?"desktop icon hit, bounded double-click, shell open, and focus passed":"icon fallback reported");putc64(c,'\n');}
...
ENTRY64 void kernel_main64_binary(struct long_mode_handoff*h){u16 c=0,n=0;task_names_keep();active_sched_class=&fair_sched_class;sched_enqueues=sched_dequeues=sched_picks=0;module_init_model();init_model_start();wait_model_start();adoption_start();resource_start();pmm_init(h);vma_init();reclaim_init();vfs_init();address_space_init(&kernel_address_space,h);
    ...
    threads[0].id=0;threads[0].state=THREAD_RUNNING;quantum_left=TIME_SLICE_TICKS;bochs_vbe_init();framebuffer_init(h);char cmd[32];u8 ch;__asm__ volatile("cli":::"memory");stack_guards_init(h);runtime_gdt_tss_init();user_thread.kernel_stack_top=runtime_tss.rsp0;user_thread.kernel_stack_bytes=PAGE_SIZE;idle_init();install_idt(h);pit_init();pic_init();clear64(&c);mouse_hw_init();pic_masks(0xf8,0xef);(void)xfce_desktop();(void)exec_validate();text64(&c,"Lesson 67: 图形桌面综合验证\nGETTICKS, GETPID, WRITE_CONSOLE, EXIT; unknown=-ENOSYS; bounded reclaim metadata\n");prompt64(&c);__asm__ volatile("sti":::"memory");for(;;){mouse_poll();if(desktop_dirty){desktop_dirty=0;(void)xfce_desktop();}else if(mouse_cursor_dirty&&framebuffer.scene_ready)xfce_cursor_update();if(gui_term_input_dirty&&shell_window_open)gui_term_input_present();if(!kbd_dequeue(&ch)){__asm__ volatile("sti; pause":::"memory");continue;}if(shell_window_open){if(ch=='\n'){gui_term_command();desktop_dirty++;}else if(ch=='\b'){if(gui_term_input_len)gui_term_input[--gui_term_input_len]=0;gui_term_input_dirty=1;}else if(ch>=32&&ch<127&&gui_term_input_len<GUI_TERM_INPUT_MAX-1){gui_term_input[gui_term_input_len++]=(char)ch;gui_term_input[gui_term_input_len]=0;gui_term_input_dirty=1;}}else if(ch=='\n'){putc64(&c,ch);cmd[n]=0;exec64(&c,h,cmd);n=0;}else if(ch=='\b'){if(n){n--;c--;VGA[c]=0x0f20;}}else if(n<31){cmd[n++]=(char)ch;putc64(&c,(char)ch);}}}
