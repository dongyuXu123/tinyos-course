...
/* These are continuation-resident, static stacks: neither is PMM-backed. */
static u8 tss_rsp0_stack[PAGE_SIZE] __attribute__((aligned(PAGE_SIZE)));
static u8 exception_ist_stack[PAGE_SIZE] __attribute__((aligned(PAGE_SIZE)));
...
static TEXT64 void idle_init(void){struct irq0_frame*f=(struct irq0_frame *)(void *)(idle_stack+PAGE_SIZE-sizeof(*f));f->r15=f->r14=f->r13=f->r11=f->r10=f->r9=f->r8=f->rdi=f->rsi=f->rbp=f->rdx=f->rcx=f->rbx=f->rax=0;f->r12=(u64)(unsigned long)(idle_stack+PAGE_SIZE);f->rip=runtime_idle_trampoline_address();f->cs=0x08;f->rflags=0x202;idle_frame=f;idle_running=0;idle_switches=idle_ticks=0;}
...
static TEXT64 u64 stack_top64(u8 *stack){return (u64)(unsigned long)(stack+PAGE_SIZE);}
static TEXT64 void runtime_gdt_tss_init(void){
    ...
    runtime_tss.reserved0=0; runtime_tss.rsp0=stack_top64(tss_rsp0_stack);
    runtime_tss.rsp1=runtime_tss.rsp2=runtime_tss.reserved1=0;
    runtime_tss.ist1=stack_top64(exception_ist_stack);
...
TEXT64 void exception_report_ist(struct exception_frame_ist*f){u16 c=0;u64 cr2=0,rsp;clear64(&c);__asm__ volatile("mov %%rsp,%0":"=r"(rsp));text64(&c,"TinyOS lesson 24 IST exception\nexception: #PF\nvector: ");hex64(&c,f->vector);text64(&c,"\nerror:  ");hex64(&c,f->error);text64(&c,"\nrip:    ");hex64(&c,f->rip);text64(&c,"\nsaved rsp: ");hex64(&c,f->rsp);text64(&c,"\nhandler rsp: ");hex64(&c,rsp);text64(&c,"\nIST1 range: ");hex64(&c,(u64)(unsigned long)exception_ist_stack);text64(&c," ");hex64(&c,runtime_tss.ist1);__asm__ volatile("mov %%cr2,%0":"=r"(cr2));text64(&c,"\ncr2:    ");hex64(&c,cr2);text64(&c,"\nCPU halted intentionally.\n");for(;;)__asm__ volatile("cli; hlt");}
