...
static u32 setup_long_mode_tables(void)
{
    ...
    if(framebuffer_ready&&long_mode_handoff.framebuffer_bytes<=PAGE_ENTRIES*PAGE_SIZE){u32 first=(u32)(long_mode_handoff.framebuffer_map/(PAGE_ENTRIES*PAGE_SIZE)),entry=(u32)((long_mode_handoff.framebuffer_map/PAGE_SIZE)%PAGE_ENTRIES),pages=(long_mode_handoff.framebuffer_bytes+PAGE_SIZE-1)/PAGE_SIZE;volatile u64*pt=(volatile u64*)(unsigned long)(u32)long_mode_handoff.pt[first];for(j=0;j<pages&&entry+j<PAGE_ENTRIES;j++)pt[entry+j]=(long_mode_handoff.framebuffer_address+(u64)j*PAGE_SIZE)|PTE_PRESENT_WRITABLE;}
