/* 第六课：在 Multiboot2 原始 memory map 上选择已保留之外的物理页。
 * Multiboot2 规定 map layout；Linux v6.12 e820.c/memblock.c 是范围规范化、
 * reservation 和早期分配的工程对照。本课只分配地址，绝不写入返回页。
 */
...
void kernel_main32(u32 magic, u32 mbi_address)
{
    ...
    printk("TinyOS lesson 6: early physical page allocator\n");
    printk("Commands: help about clear mmap pinfo palloc. Allocations are address-only.\n\n");
    if (prepare_memory_map()) phys_allocator_init(); else printk("warning: Multiboot2 map is invalid\n");
