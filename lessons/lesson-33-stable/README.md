# Lesson 33: bounded address-space object over the validated kernel-embedded user image
...
Run `vmtest`, then try `vmap 0xffffffff80000000 <phys>`, duplicate `vmap`, and duplicate `vunmap` to observe validation. Run `idtinfo`, then `cpl3test` from a fresh boot. The banner and `about` text identify Lesson 33 and the image validation/load boundary.
