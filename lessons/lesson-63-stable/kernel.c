...
#define QEMU_STD_FB 0xfd000000ULL
#define QEMU_STD_FB_PITCH 3200U
#define QEMU_STD_FB_WIDTH 800U
#define QEMU_STD_FB_HEIGHT 600U
...
static int prepare_memory_map(void)
{
    ...
    while(pos<end) { const struct mb2_tag *tag; u32 rounded;
        ...
        if(tag->type==MB2_TAG_END) { u64 bytes; if(tag->size!=8 || !memory_map) return 0; if(framebuffer_tag && framebuffer_tag->bpp==32 && framebuffer_tag->type_field==1 && !(framebuffer_tag->address&(PAGE_SIZE-1)) && framebuffer_tag->pitch>=framebuffer_tag->width*4U && framebuffer_tag->height && (bytes=(u64)framebuffer_tag->pitch*framebuffer_tag->height)<=PAGE_ENTRIES*PAGE_SIZE && framebuffer_tag->address+bytes>=framebuffer_tag->address){long_mode_handoff.framebuffer_address=framebuffer_tag->address;long_mode_handoff.framebuffer_pitch=framebuffer_tag->pitch;long_mode_handoff.framebuffer_width=framebuffer_tag->width;long_mode_handoff.framebuffer_height=framebuffer_tag->height;long_mode_handoff.framebuffer_bpp=framebuffer_tag->bpp;long_mode_handoff.framebuffer_type=framebuffer_tag->type_field;long_mode_handoff.framebuffer_bytes=(u32)bytes;long_mode_handoff.framebuffer_map=FRAMEBUFFER_VA;framebuffer_ready=1;} else { framebuffer_ready=0; } memory_map_ready=1; return 1; }
...
static u32 setup_long_mode_tables(void)
{
    ...
    if(framebuffer_ready){u32 first=(u32)(long_mode_handoff.framebuffer_map/(PAGE_ENTRIES*PAGE_SIZE)),entry=(u32)((long_mode_handoff.framebuffer_map/PAGE_SIZE)%PAGE_ENTRIES),pages=(long_mode_handoff.framebuffer_bytes+PAGE_SIZE-1)/PAGE_SIZE,k;for(k=0;k<pages;k++){u32 slot=first+(entry+k)/PAGE_ENTRIES;u32 off=(entry+k)%PAGE_ENTRIES;if(slot>=PAGE_TABLES_PER_ALIAS){framebuffer_ready=0;break;}((volatile u64 *)(unsigned long)(u32)long_mode_handoff.pt[slot])[off]=(long_mode_handoff.framebuffer_address+(u64)k*PAGE_SIZE)|PTE_PRESENT_WRITABLE;((volatile u64 *)(unsigned long)(u32)long_mode_handoff.high_pt[slot])[off]=(long_mode_handoff.framebuffer_address+(u64)k*PAGE_SIZE)|PTE_PRESENT_WRITABLE;}long_mode_handoff.framebuffer_map=FRAMEBUFFER_VA;}
