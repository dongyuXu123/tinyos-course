...
/* Lesson 36 keeps two bounded process/address-space/thread objects.  The
...
static TEXT64 void user_irq0_save_restore(struct irq0_frame *f)
{
    ...
    /* The active bounded user thread is the only legal destination: restore the exact
...
static TEXT64 int user_process_exit(void)
{
    ...
    user_process.state=PROCESS_EXITED; user_thread.state=USER_THREAD_EXITED; user_thread.transitions++; user_exit_count++; return 1;
}
...
static TEXT64 void processinfo(u16*c)
{
    ...
    text64(c,"\ncontext lifecycle: ");text64(c,user_process.context_valid?"validated":"not validated");text64(c," transitions ");hex64(c,user_thread.transitions);text64(c,"\nsecond pid/state/as/thread: ");hex64(c,user_processes[1].pid);text64(c," ");text64(c,process_state_name(user_processes[1].state));text64(c," ");hex64(c,(u64)(unsigned long)user_processes[1].address_space);text64(c," ");hex64(c,user_threads[1].tid);text64(c," ");text64(c,user_thread_state_name(user_threads[1].state));text64(c,"\nprograms bounded: ");hex64(c,MAX_USER_PROGRAMS);text64(c," exits/reclaims: ");hex64(c,user_exit_count);text64(c," ");hex64(c,user_reclaims);putc64(c,'\n');
}
static TEXT64 int process_lifecycle_test(u16*c)
{
    ...
    int ok=user_programs_ready() && user_process.state==PROCESS_READY && user_thread.state==USER_THREAD_READY &&
        user_process.address_space==&user_address_spaces[0] && user_process.code_phys==user_code_phys &&
    ...
    text64(c,"process lifecycle: ");text64(c,ok?"two bounded process/address-space/thread objects ready":"BROKEN");putc64(c,'\n');
    return ok;
}
...
ENTRY64 void kernel_main64_binary(struct long_mode_handoff*h){u16 c=0,n=0;pmm_init(h);address_space_init(&kernel_address_space,h); user_address_spaces[0]=kernel_address_space; user_address_spaces[1]=kernel_address_space;
    user_process.pid=FIXED_PID; user_process.address_space=&user_address_spaces[0]; user_process.code_phys=user_code_phys; user_process.stack_phys=user_stack_phys;
