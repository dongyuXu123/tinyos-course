...
static TEXT64 void task_model_init(void){u32 i;for(i=0;i<TASK_TABLE_CAP;i++){task_table[i].pid=i?((u64)i+1):0;task_table[i].tid=task_table[i].pid;task_table[i].parent_pid=i?1:0;task_table[i].kind=i?TASK_KIND_USER:TASK_KIND_KERNEL;task_table[i].state=TASK_RUNNING;task_table[i].transitions=0;task_table[i].valid=1;}task_table[1].parent_pid=0;}
...
static TEXT64 void tasklist(u16*c){u32 i;text64(c,"tasks: slot pid tid parent kind state transitions\n");for(i=0;i<TASK_TABLE_CAP;i++)if(task_table[i].valid){struct task_struct*t=&task_table[i];text64(c,"task ");hex64(c,i);text64(c," ");hex64(c,t->pid);text64(c," ");hex64(c,t->tid);text64(c," ");hex64(c,t->parent_pid);text64(c," ");text64(c,task_kind_name(t->kind));text64(c," ");text64(c,task_state_name(t->state));text64(c," ");hex64(c,t->transitions);putc64(c,'\n');}}
...
static TEXT64 int fault_insert(u64 va,u8 write){u32 i;u64 p;if(fault_page_count>=VMA_MAX_PAGES||!vma_range_valid(down(va),down(va)+PAGE_SIZE,write?VMA_W:VMA_R)||page_present(va))return 0;p=pmm_alloc();if(!p)return 0;for(i=0;i<VMA_MAX_PAGES;i++)if(!fault_pages[i].live){fault_pages[i].va=down(va);fault_pages[i].phys=p;fault_pages[i].writable=write;fault_pages[i].live=1;fault_page_count++;fault_insertions++;return 1;}return 0;}
...
static TEXT64 void vmatest(u16*c){int ok=vma_range_valid(VMA_DATA_START,VMA_DATA_START+PAGE_SIZE,VMA_W)&&!vma_range_valid(VMA_CODE_START,VMA_CODE_START+PAGE_SIZE,VMA_W)&&!vma_lookup(0x00100000ULL);text64(c,"vmatest: ");text64(c,ok?"lookup/protection/range validation passed":"BROKEN");putc64(c,'\n');}
