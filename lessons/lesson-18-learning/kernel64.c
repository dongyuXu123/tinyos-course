/* Lesson 18: PIT preemptive scheduling at the IRQ0 return boundary. */
...
/* Thread 0 is the shell. Workers own one PMM frame each and run only after yield. */
...
static TEXT64 const char *thread_state_name(u8 state){if(state==THREAD_RUNNING)return "running";if(state==THREAD_RUNNABLE)return "runnable";if(state==THREAD_FINISHED)return "finished";return "empty";}
