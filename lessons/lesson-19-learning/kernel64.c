/* Lesson 19: PIT timer sleep/wakeup at the IRQ0 return boundary. */
...
enum thread_state { THREAD_EMPTY, THREAD_RUNNING, THREAD_RUNNABLE, THREAD_SLEEPING, THREAD_FINISHED };
...
static TEXT64 const char *thread_state_name(u8 state){if(state==THREAD_RUNNING)return "running";if(state==THREAD_RUNNABLE)return "runnable";if(state==THREAD_SLEEPING)return "sleeping";if(state==THREAD_FINISHED)return "finished";return "empty";}
...
static TEXT64 void worker_run(u8 id){while(threads[id].progress<THREAD_STEPS){threads[id].progress++;if(sleep_test)thread_sleep_ticks(id==1?SLEEP_A_TICKS:SLEEP_B_TICKS);else busy_delay();}thread_exit();}
