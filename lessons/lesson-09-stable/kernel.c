/* 第九课 32-bit setup：验证 Multiboot2，并交接 long-mode page tables 与 IDT backing store。 */
...
/* 此块与 IDT 均经 RDI 交给 .text64；所有地址在本课 identity window 内。 */
...
static int page_was_allocated(u64 p) { u32 i; for(i=0;i<long_mode_handoff.allocated_pages;i++) if(long_mode_handoff.allocation_history[i]==p) return 1; return 0; }
static int page_is_reserved(u64 p)
{
    u64 end=p+PAGE_SIZE;
    if(end<p || ranges_overlap(p,end,0,LOW_MEMORY_END)) return 1;
    if(ranges_overlap(p,end,long_mode_handoff.kernel_start,long_mode_handoff.kernel_end)) return 1;
    if(ranges_overlap(p,end,long_mode_handoff.stack_start,long_mode_handoff.stack_end)) return 1;
    if(ranges_overlap(p,end,multiboot_address,(u64)multiboot_address+multiboot_total_size)) return 1;
    return page_was_allocated(p);
}
...
static u64 phys_alloc_page(void)
{
    ...
    for(offset=0;offset<memory_map->size-16;offset+=memory_map->entry_size) { const struct mb2_mmap_entry *e=(const struct mb2_mmap_entry *)((const u8 *)memory_map+16+offset); u64 p,end; if(e->type!=1 || e->addr+e->len<e->addr) continue; p=align_up_page(e->addr); end=align_down_page(e->addr+e->len); while(p && p<end) { if(!page_is_reserved(p)) { long_mode_handoff.allocation_cursor=p+PAGE_SIZE; long_mode_handoff.allocation_end=end; long_mode_handoff.allocation_history[long_mode_handoff.allocated_pages++]=p; return p; } p+=PAGE_SIZE; } }
    return 0;
}
