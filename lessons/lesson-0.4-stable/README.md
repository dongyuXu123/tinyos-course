# GRUB 研读 0.4：GRUB ELF 装载器路径

> **Course status: stable study note.** 这是文档和工具分析课，不生成 TinyOS 内核。

## 学习目标

multiboot2 命令、ELF program header、PT_LOAD、entry transfer，并对照 TinyOS linker.ld。
...
上一节点：`0.3`。下一节点：`0.5`。完成本课后进入下一节点；0.10 完成后进入 Lesson 01。
