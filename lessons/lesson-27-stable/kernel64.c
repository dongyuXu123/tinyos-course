/* Lesson 27: bounded final-PT dual-alias mapping registry and ownership. */
...
struct long_mode_handoff { u64 pml4,pdpt,pd,idt_address,pt[PAGE_TABLES_PER_ALIAS]; u64 kernel_start,kernel_end,stack_start,stack_end; u64 high_pdpt,high_pd,high_pt[PAGE_TABLES_PER_ALIAS]; u64 kernel_vma_base,kernel_phys_base; u32 mbi_address,mbi_size; };
...
_Static_assert(sizeof(struct irq0_frame)==18*sizeof(u64),"IRQ0 frame");
...
static TEXT64 int pmm_reserved(struct long_mode_handoff*h,u64 p){u64 e=p+PAGE_SIZE,b=(u64)(unsigned long)pmm_bitmap-KERNEL_VMA_BASE,z=b+2*PMM_BITMAP_BYTES;u32 i;if(p<0x100000||e<p||overlap(p,e,h->kernel_start,h->kernel_end)||overlap(p,e,h->stack_start,h->stack_end)||overlap(p,e,h->mbi_address,(u64)h->mbi_address+h->mbi_size)||overlap(p,e,h->pml4,h->pml4+PAGE_SIZE)||overlap(p,e,h->pdpt,h->pdpt+PAGE_SIZE)||overlap(p,e,h->pd,h->pd+PAGE_SIZE)||overlap(p,e,h->high_pdpt,h->high_pdpt+PAGE_SIZE)||overlap(p,e,h->high_pd,h->high_pd+PAGE_SIZE)||overlap(p,e,h->idt_address,h->idt_address+PAGE_SIZE)||overlap(p,e,b,z))return 1;for(i=0;i<PAGE_TABLES_PER_ALIAS;i++)if(overlap(p,e,h->pt[i],h->pt[i]+PAGE_SIZE)||overlap(p,e,h->high_pt[i],h->high_pt[i]+PAGE_SIZE))return 1;return 0;}
...
static TEXT64 int vm_slot(u64 va,u32 *slot){if((va&(PAGE_SIZE-1))||va<VM_REGION_START||va>=VM_REGION_END)return 0;*slot=(u32)((va-VM_REGION_START)/PAGE_SIZE);return 1;}
...
static TEXT64 int vm_frame_owned(u64 p){u32 i;for(i=0;i<VM_REGION_SLOTS;i++)if(vm_mappings[i].live&&vm_mappings[i].phys==p)return 1;return 0;}
...
static TEXT64 const char *map_page(struct long_mode_handoff*h,u64 va,u64 p){volatile u64*l,*q;u64 flags;u32 slot;const char*s;if(!vm_slot(va,&slot))return "VA outside mapping region";s=page_state(p);if(!eq64(s,"allocated"))return s;flags=irq_save64();l=vm_pte_low(h,slot);q=vm_pte_high(h,slot);if(!vm_pair_ok(h,slot)){irq_restore64(flags);return "inconsistent PTE pair";}if(vm_mappings[slot].live||*l){irq_restore64(flags);return "slot already mapped";}if(vm_frame_owned(p)){irq_restore64(flags);return "frame already mapped";}*l=*q=p|PTE_PRESENT_WRITABLE;vm_mappings[slot].phys=p;vm_mappings[slot].live=1;invlpg64(va);invlpg64(KERNEL_VMA_BASE+va);irq_restore64(flags);return "mapped";}
static TEXT64 const char *unmap_page(struct long_mode_handoff*h,u64 va){volatile u64*l,*q;u64 flags;u32 slot;if(!vm_slot(va,&slot))return "VA outside mapping region";flags=irq_save64();l=vm_pte_low(h,slot);q=vm_pte_high(h,slot);if(!vm_pair_ok(h,slot)){irq_restore64(flags);return "inconsistent PTE pair";}if(!vm_mappings[slot].live){irq_restore64(flags);return "slot already unmapped";}*l=*q=0;vm_mappings[slot].phys=0;vm_mappings[slot].live=0;invlpg64(va);invlpg64(KERNEL_VMA_BASE+va);irq_restore64(flags);return "unmapped";}
static TEXT64 void vminfo(u16*c,struct long_mode_handoff*h,u64 va,int one){u32 i,n=0,slot;if(one&&!vm_slot(va,&slot)){text64(c,"VM VA outside mapping region\n");return;}for(i=0;i<VM_REGION_SLOTS;i++)if(vm_mappings[i].live)n++;text64(c,"VM region low/high: ");hex64(c,VM_REGION_START);text64(c," ");hex64(c,VM_REGION_HIGH_START);text64(c,"\nslots live/total: ");hex64(c,n);text64(c," ");hex64(c,VM_REGION_SLOTS);if(one){text64(c,"\nVA low/high: ");hex64(c,va);text64(c," ");hex64(c,KERNEL_VMA_BASE+va);text64(c,"\nstate/owner: ");text64(c,vm_mappings[slot].live?"mapped ":"unmapped ");hex64(c,vm_mappings[slot].phys);text64(c,"\nPTE low/high: ");hex64(c,*vm_pte_low(h,slot));text64(c," ");hex64(c,*vm_pte_high(h,slot));}putc64(c,'\n');}
...
static TEXT64 void install_idt(struct long_mode_handoff*h){struct idt_gate *idt=(struct idt_gate *)(unsigned long)phys_to_high(h->idt_address);struct idtr d;u32 i;for(i=0;i<IDT_ENTRIES;i++){idt[i].offset_low=0;idt[i].selector=0;idt[i].ist=0;idt[i].type=0;idt[i].offset_mid=0;idt[i].offset_high=0;idt[i].reserved=0;}set_gate(&idt[3],runtime_bp_address(),0);set_gate(&idt[6],runtime_ud_address(),0);set_gate(&idt[14],runtime_pf_address(),IST1_INDEX);set_gate(&idt[0x20],runtime_irq0_address(),0);set_gate(&idt[0x21],runtime_irq1_address(),0);d.limit=sizeof(struct idt_gate)*IDT_ENTRIES-1;d.base=(u64)(unsigned long)idt;__asm__ volatile("lidt %0"::"m"(d):"memory");}
...
static TEXT64 void tssinfo(u16*c,struct long_mode_handoff*h){u16 tr;struct idt_gate *idt=(struct idt_gate *)(unsigned long)phys_to_high(h->idt_address);__asm__ volatile("str %0":"=r"(tr));text64(c,"TSS/IST: kernel-only transition preparation\nTR: ");hex64(c,tr);text64(c,"\nGDTR base/limit: ");hex64(c,runtime_gdtr.base);text64(c," ");hex64(c,runtime_gdtr.limit);text64(c,"\nTSS base: ");hex64(c,(u64)(unsigned long)&runtime_tss);text64(c,"\nrsp0 top: ");hex64(c,runtime_tss.rsp0);text64(c,"\nIST1 top: ");hex64(c,runtime_tss.ist1);text64(c,"\n#PF IST: ");hex64(c,idt[14].ist&7);text64(c,"\nIRQ0/IRQ1 IST: ");hex64(c,idt[0x20].ist&7);text64(c," ");hex64(c,idt[0x21].ist&7);text64(c,"\nCPL3 entry: not implemented\n");}
