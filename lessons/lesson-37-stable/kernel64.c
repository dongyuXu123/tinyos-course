...
ENTRY64 void kernel_main64_binary(struct long_mode_handoff*h){u16 c=0,n=0;pmm_init(h);address_space_init(&kernel_address_space,h);
    ...
    task_table[0].pid=0; task_table[0].tid=0; task_table[0].parent_pid=0; task_table[0].kind=TASK_KIND_KERNEL; task_table[0].state=TASK_RUNNING; task_table[0].valid=1;
    task_table[1].pid=FIXED_PID; task_table[1].tid=FIXED_PID; task_table[1].parent_pid=0; task_table[1].kind=TASK_KIND_USER; task_table[1].state=TASK_INTERRUPTIBLE; task_table[1].valid=1;
    task_table[2].pid=SECOND_PID; task_table[2].tid=SECOND_PID; task_table[2].parent_pid=0; task_table[2].kind=TASK_KIND_USER; task_table[2].state=TASK_INTERRUPTIBLE; task_table[2].valid=1;
    task_table[3].pid=3; task_table[3].tid=3; task_table[3].parent_pid=0; task_table[3].kind=TASK_KIND_KERNEL; task_table[3].state=TASK_RUNNING; task_table[3].valid=1;
    if(!task_table_validate()) for(;;)__asm__ volatile("cli; hlt");
    threads[0].id=0;threads[0].state=THREAD_RUNNING;quantum_left=TIME_SLICE_TICKS;char cmd[32];u8 ch;__asm__ volatile("cli":::"memory");stack_guards_init(h);runtime_gdt_tss_init();user_thread.kernel_stack_top=runtime_tss.rsp0;user_thread.kernel_stack_bytes=PAGE_SIZE;idle_init();install_idt(h);pit_init();pic_init();clear64(&c);text64(&c,"TinyOS lesson 36: validated user image with bounded PIT preemption\nGETTICKS, GETPID, WRITE_CONSOLE, EXIT; unknown=-ENOSYS; all-GPR CPL3 IRQ0 frame; IF policy preserved\n");prompt64(&c);__asm__ volatile("sti":::"memory");for(;;){if(!kbd_dequeue(&ch)){__asm__ volatile("sti; hlt":::"memory");continue;}if(ch=='\n'){putc64(&c,ch);cmd[n]=0;exec64(&c,h,cmd);n=0;}else if(ch=='\b'){if(n){n--;c--;VGA[c]=0x0f20;}}else if(n<31){cmd[n++]=(char)ch;putc64(&c,(char)ch);}}}
