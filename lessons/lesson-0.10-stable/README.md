# GRUB 研读 0.10：GRUB → TinyOS 端到端 checkpoint

> **Course status: stable study note.** 这是文档和工具分析课，不生成 TinyOS 内核。

## 学习目标

把 BIOS/El Torito、parser、filesystem、ELF loader、MBI 和 boot.S 连接成完整时序，下一步进入 Lesson 01。
...
上一节点：`0.9`。下一节点：`Lesson 01`。完成本课后进入下一节点；0.10 完成后进入 Lesson 01。
