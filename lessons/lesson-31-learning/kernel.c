/* Lesson 31: reserve 16 final-PT slots for synchronized low/high mappings. */
...
struct long_mode_handoff {
    ...
    u64 user_code_phys, user_stack_phys;
...
static u32 setup_long_mode_tables(void)
{
    ...
    long_mode_handoff.user_code_phys=bootstrap_alloc_page(); long_mode_handoff.user_stack_phys=bootstrap_alloc_page();
    if(!table_page_ok(long_mode_handoff.user_code_phys)||!table_page_ok(long_mode_handoff.user_stack_phys))return 0;
    { volatile u8 *code=(volatile u8 *)(unsigned long)(u32)long_mode_handoff.user_code_phys; volatile u64 *pt=(volatile u64 *)(unsigned long)(u32)long_mode_handoff.pt[USER_CODE_VA/(PAGE_ENTRIES*PAGE_SIZE)]; volatile u64 *st=(volatile u64 *)(unsigned long)(u32)long_mode_handoff.pt[USER_STACK_VA/(PAGE_ENTRIES*PAGE_SIZE)]; u32 i=0; u32 calls[5]={0,1,2,99,3}; for(;i<5;i++){code[i*7]=0xb8;code[i*7+1]=(u8)calls[i];code[i*7+2]=(u8)(calls[i]>>8);code[i*7+3]=(u8)(calls[i]>>16);code[i*7+4]=(u8)(calls[i]>>24);code[i*7+5]=0xcd;code[i*7+6]=0x80;} code[35]=0xeb;code[36]=0xfe; pt[(USER_CODE_VA/PAGE_SIZE)%PAGE_ENTRIES]=long_mode_handoff.user_code_phys|PTE_PRESENT|PTE_USER; st[(USER_STACK_VA/PAGE_SIZE)%PAGE_ENTRIES]=long_mode_handoff.user_stack_phys|PTE_PRESENT_WRITABLE|PTE_USER; }
