# GRUB 研读 0.1：GRUB 源码树与启动产物

> **Course status: stable study note.** 这是文档和工具分析课，不生成 TinyOS 内核。

## 学习目标

grub-core、include/grub、conf、util 的职责；BIOS core image、模块、El Torito ISO、grub-mkrescue 和 grub-file。
...
上一节点：`Lesson 00`。下一节点：`0.2`。完成本课后进入下一节点；0.10 完成后进入 Lesson 01。
