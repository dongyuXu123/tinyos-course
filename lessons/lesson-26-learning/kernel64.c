/* Lesson 21: bounded wait queues with wake-one and wake-all. */
...
#define DYNAMIC_TEST_VA 0x00fff000ULL
#define DYNAMIC_TEST_HIGH_VA (KERNEL_VMA_BASE+DYNAMIC_TEST_VA)
#define PAGE_TABLES_PER_ALIAS 8U
#define DYNAMIC_PT_INDEX (PAGE_TABLES_PER_ALIAS-1)
...
static TEXT64 volatile u64 *vm_pte_low(struct long_mode_handoff*h){return &((volatile u64 *)(unsigned long)h->pt[DYNAMIC_PT_INDEX])[DYNAMIC_PT1_INDEX];}
static TEXT64 volatile u64 *vm_pte_high(struct long_mode_handoff*h){return &((volatile u64 *)(unsigned long)h->high_pt[DYNAMIC_PT_INDEX])[DYNAMIC_PT1_INDEX];}
static TEXT64 const char *map_page(struct long_mode_handoff*h,u64 p){volatile u64*l,*q;u64 flags;const char*s=page_state(p);if(!eq64(s,"allocated"))return s;flags=irq_save64();l=vm_pte_low(h);q=vm_pte_high(h);if(vm_window_phys||*l||*q){irq_restore64(flags);return "slot already mapped";}*l=*q=p|PTE_PRESENT_WRITABLE;invlpg64(DYNAMIC_TEST_VA);invlpg64(DYNAMIC_TEST_HIGH_VA);vm_window_phys=p;irq_restore64(flags);return "mapped";}
static TEXT64 const char *unmap_page(struct long_mode_handoff*h){volatile u64*l,*q;u64 flags;if(!vm_window_phys)return "slot already unmapped";flags=irq_save64();l=vm_pte_low(h);q=vm_pte_high(h);*l=*q=0;invlpg64(DYNAMIC_TEST_VA);invlpg64(DYNAMIC_TEST_HIGH_VA);vm_window_phys=0;irq_restore64(flags);return "unmapped";}
static TEXT64 void vminfo(u16*c,struct long_mode_handoff*h){text64(c,"VM low/high: ");hex64(c,DYNAMIC_TEST_VA);text64(c," ");hex64(c,DYNAMIC_TEST_HIGH_VA);text64(c,"\nstate/owner: ");text64(c,vm_window_phys?"mapped ":"unmapped ");hex64(c,vm_window_phys);text64(c,"\nPTE low/high: ");hex64(c,*vm_pte_low(h));text64(c," ");hex64(c,*vm_pte_high(h));putc64(c,'\n');}
