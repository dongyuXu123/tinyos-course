# GRUB 研读 0.2：grub.cfg 配置语言与命令分发

> **Course status: stable study note.** 这是文档和工具分析课，不生成 TinyOS 内核。

## 学习目标

menuentry、multiboot2、boot、timeout/default，以及 normal、script 和 command 注册路径。
...
上一节点：`0.1`。下一节点：`0.3`。完成本课后进入下一节点；0.10 完成后进入 Lesson 01。
